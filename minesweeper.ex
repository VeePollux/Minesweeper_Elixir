defmodule Minesweeper do
  ...
  def valid_moves(tam,l,c) do
    posicoes = [{l-1, c-1}, {l-1, c}, {l-1, c+1},
                {l, c-1},             {l, c+1},
                {l+1, c-1}, {l+1, c}, {l+1, c+1}]
    Enum.filter(posicoes, fn {linhas, colunas} -> is_valid_pos(tam, linhas, colunas) end)
  end
  ...
  def conta_minas_adj(tab, l,c), do: conta_minas_adj(tab, valid_moves(length(tab), l, c))
  def conta_minas_adj(_tab,[]), do: 0

  def conta_minas_adj(tab, [{l,c}|t]) do
    cond do
      is_mine(tab, l, c) -> conta_minas_adj(tab, t) + 1
      true -> conta_minas_adj(tab, t)
    end
  end
  ...
  def is_open(tab, l, c), do: get_pos(tab,l,c) != "-"

  def abre_jogada([{l,c}|t], minas, tab), do:
  abre_jogada(t,minas,abre_jogada(l,c,minas,tab))
  def abre_jogada([],_minas,tab), do: tab
  def abre_jogada(l,c,minas,tab) do
     cond do
       is_mine(minas, l, c) -> tab
       is_open(tab, l, c) -> tab
       conta_minas_adj(minas,l,c) == 0 -> abre_jogada(valid_moves(length(minas), l, c),minas, update_pos(tab,l,c,conta_minas_adj(minas,l,c)))
...
# todo o tabuleiro no caso de vitória ou derrota. Para implementar esta função, usar a função abre_posicao/4
  def abre_tabuleiro(minas,tab, 0, 0), do: abre_posicao(tab,minas,0,0)
  def abre_tabuleiro(minas, tab, -1, c), do: abre_tabuleiro(minas, tab, length(tab)-1, c-1)
  def abre_tabuleiro(minas,tab, l, c), do: abre_tabuleiro(minas,  abre_posicao(tab,minas,l, c), l-1, c)
  def abre_tabuleiro(minas,tab), do: abre_tabuleiro(minas, tab, length(minas)-1, length(minas)-1)
...
def board_to_string(tab) do
  qt_colunas = length(tab)
  colunas = Enum.join(0..(qt_colunas - 1), "   ")
  IO.puts(IO.ANSI.green() <> "     " <> colunas)
  IO.puts(IO.ANSI.green()<>"   " <> String.duplicate("_", qt_colunas + (qt_colunas * 3)))

  tab
  |> Enum.with_index()
  |> Enum.each(fn {linha, i} ->
    linha_str = Enum.join(linha, "   ")
    IO.puts(IO.ANSI.green() <> "#{i}  | " <> IO.ANSI.light_black() <> linha_str)
  end)

  IO.puts(IO.ANSI.green() <> "   " <> String.duplicate("-", qt_colunas + (qt_colunas * 3)))
end
...
# conta_fechadas/1: recebe um tabueleiro de jogo e conta quantas posições fechadas existem no tabuleiro (posições com "-")
  ...
  def conta_fechadas(tab), do: conta_fechadas(tab, length(tab)-1, length(tab)-1)
  def conta_fechadas(_tab, -1, 0), do: 0
  def conta_fechadas(tab, -1, c), do: conta_fechadas(tab, length(tab)-1, c-1)
  def conta_fechadas(tab, l, c) do
    cond do
      !is_open(tab,l,c) -> conta_fechadas(tab, l-1, c) + 1
      true -> conta_fechadas(tab, l-1, c)
    end
  end

# -- conta_minas/1: Recebe o tabuleiro de Minas (MBoard) e conta quantas minas existem no jogo

  def conta_minas(minas), do: conta_minas(minas, length(minas)-1, length(minas)-1)
  def conta_minas(_minas, -1, 0), do: 0
  def conta_minas(minas, -1, c), do: conta_minas(minas, length(minas)-1, c-1)
  def conta_minas(minas, l, c) do
    cond do
      is_mine(minas, l, c) -> conta_minas(minas, l-1, c) + 1
      true -> conta_minas(minas, l-1, c)
    end
  end
